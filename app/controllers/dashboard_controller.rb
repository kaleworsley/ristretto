...
=begin
    ...
    when 'assigned_tasks'
      @tasks = Task.paginate(:per_page => 50, :page => params[:page], :conditions => {:tasks => {:state => Task.stategroups[:current], :assigned_to_id => current_user.id}, :projects => {:state => 'current'}}, :include => :project, :order => 'projects.name ASC, tasks.name ASC')
    ...
    when 'not_staff_dashboard'
      @tasks = Task.find(:all, :conditions => {:tasks => {:state => 'delivered'}, :projects => {:state => 'current', :id => current_user.current_projects_ids}}, :include => :project, :order => 'projects.name ASC, tasks.name ASC').find_all {|task| task.project.stakeholders.find(:all, :conditions => {:role => 'customer_representative', :user_id => current_user.id})}
      @projects = current_user.current_projects.current.find(:all, :order => :name)
